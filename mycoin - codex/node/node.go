...
type BlockBroadcaster interface {
	...
	StrongestPeerSyncTarget() (uint64, *big.Int, string)
}
...
func (n *Node) Mine() {
	...
	for {
		...
		if n.Broadcaster != nil {
			peerHeight, peerWork, peerAddr := n.Broadcaster.StrongestPeerSyncTarget()
			...
			if n.Best != nil {
				...
				if n.Best.CumWorkInt != nil {
					localWork = new(big.Int).Set(n.Best.CumWorkInt)
				}
			}
			n.mu.Unlock()

			if peerWork != nil && peerWork.Cmp(localWork) > 0 {
				n.EvaluateSyncStatus(peerHeight, new(big.Int).Set(peerWork))
				fmt.Printf("[Sync] delaying mining because peer %s advertises more work (local height %d, peer height %d)\n", peerAddr, localHeight, peerHeight)
				time.Sleep(2 * time.Second)
				continue
			}
		}
